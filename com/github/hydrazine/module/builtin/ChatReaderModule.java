...
public class ChatReaderModule implements Module {
	...
	public void start() {
		...
		// User forgot to pass the options
			...
			System.out.println(Hydrazine.errorPrefix
					+ "No client option specified. You have to append one of those switches to the command: -u, -gu or -cr");
		}
	}
	...
	public void configure() {
		...
		settings.setProperty("commandDelay",
				ModuleSettings.askUser("Enter the delay between the commands in milliseconds: "));
			...
			public void packetReceived(PacketReceivedEvent event) {
				if (event.getPacket() instanceof ServerJoinGamePacket) {
					if (settings.containsKey("loginCommand") && settings.containsKey("registerCommand")) {
						if (!(settings.getProperty("loginCommand").isEmpty()
								&& settings.getProperty("registerCommand").isEmpty())) {
					...
					if (packet.getType() != MessageType.NOTIFICATION) {
						if (settings.containsKey("filterColorCodes")
								&& settings.getProperty("filterColorCodes").equals("true")) {
								...
								if (msg.getTranslationKey().startsWith("chat.type")) {
									...
									if (msg.getTranslationKey().endsWith("left")) {
										message = String.format("%s left the game.",
												(Object[]) msg.getTranslationParams());
									} else if (msg.getTranslationKey().endsWith("joined")) {
										message = String.format("%s joined the game.",
												(Object[]) msg.getTranslationParams());
									}
								}
							...
							if (packet.getMessage() instanceof TranslationMessage) {
								...
								if (msg.getTranslationKey().startsWith("chat.type")) {
									...
									String message = String.format("[PM] <%s> %s",
											(Object[]) msg.getTranslationParams());

									System.out.println(message);
								} else if (msg.getTranslationKey().startsWith("multiplayer.player")) {
									if (msg.getTranslationKey().endsWith("left")) {
										String message = String.format("%s left the game.",
												(Object[]) msg.getTranslationParams());

										System.out.println(message);
									} else if (msg.getTranslationKey().endsWith("joined")) {
										String message = String.format("%s joined the game.",
												(Object[]) msg.getTranslationParams());
