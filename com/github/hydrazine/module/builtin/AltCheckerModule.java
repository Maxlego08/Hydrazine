...
public class AltCheckerModule implements Module {
	...
	public void start() {
		...
		if (settings.containsKey("loadFromFile") && settings.getProperty("loadFromFile").equals("true")) {
			...
			System.out.println(Hydrazine.errorPrefix
					+ "You have to either configure the module to use a list (-c) or use the -cr switch to check a single account. (Format: username/email:password)");
		}
	}
	...
	public void configure() {
		...
		if (Boolean.valueOf(settings.getProperty("loadFromFile"))) {
			...
			settings.setProperty("loginDelay",
					String.valueOf(ModuleSettings.askUser("Delay between login attempts (in milliseconds):")));
		}

		settings.setProperty("outputToFile",
				String.valueOf(ModuleSettings.askUserYesNo("Output working accounts to file?")));
