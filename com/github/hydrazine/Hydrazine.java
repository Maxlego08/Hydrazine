...
public class Hydrazine {
	...
	public static void main(String[] args) {
		...
		System.out
				.println("     | |  | | |_| | (_| | | | (_| |/ /| | | | |  __/     v" + progVer + " (Minecraft 1.8.8)");
		...
		try {
			...
			System.out.println(
					Hydrazine.errorPrefix + "Missing host (-h), missing module (-m) or invalid option passed.\n");
		...
		if (cmd.hasOption('p')) {
			...
			try {
				...
				System.out.println(Hydrazine.errorPrefix
						+ "The specified port is not a valid number. Using default port. (25565)");
			}
		...
		System.out.println(Hydrazine.infoPrefix + "Starting Hydrazine v" + Hydrazine.progVer + " at "
				+ new Date().toString() + "\n");
		...
		if (!settings.getSetting("module").contains(".jar")) {
			...
			for (Module m : loadedModules) {
				if (m.getName().equalsIgnoreCase(settings.getSetting("module"))) {
					if (cmd.hasOption('c')) // Configure module if '-c' switch
											// is present
					{
						m.configure();
			...
			if (!foundModule) {
				System.out.println(
						Hydrazine.warnPrefix + "Couldn't find module \"" + settings.getSetting("module") + "\"");
			}
		} else // Start external module
		{
			...
			if (m != null) {
				if (cmd.hasOption('c')) // Configure module if '-c' switch is
										// present
				{
					m.configure();
	...
	private static void registerOptions(Options options) {
		...
		Option accOpt = new Option("cr", "credentials", true,
				"Credentials of a minecraft account. (Format: username/email:password)");
		accOpt.setArgName("creds");
		Option aProxyOpt = new Option("ap", "auth-proxy", true,
				"A proxy or a file containing proxies, used for authentication. Format: host:port (https)");
		aProxyOpt.setArgName("proxy");
		Option sProxyOpt = new Option("sp", "socks-proxy", true,
				"A proxy or a file containing proxies, used to connect to a server. Format: host:port (socks)");
	...
	private static void listModules() {
		...
		System.out.println("\n" + Hydrazine.infoPrefix
				+ "To run an external module, please specify the path of the jar file by using the switch \"-m\"");
	}
	...
	/*
	 * Gets the modules from the directory, specified by the "hydrazine"
	...
	private static Module[] getModulesFromEnvVar(ModuleManager mm) {
		...
		for (File f : ModuleManager.getJarFilesFromDir(dir)) {
			try {
				...
				System.out.println(
						Hydrazine.warnPrefix + f.getAbsolutePath() + " isn't a valid module, you should remove it.");
			}
		}
