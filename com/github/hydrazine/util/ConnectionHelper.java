 ...
 * @author xTACTIXzZ
 * 
 *         This class helps connecting a client to a server. It respects the
		...
		client.getSession().addListener(new SessionAdapter() {
			...
			public void packetReceived(PacketReceivedEvent event) {
				if (event.getPacket() instanceof ServerJoinGamePacket) {
					System.out.println(Hydrazine.infoPrefix
							+ ((MinecraftProtocol) client.getPacketProtocol()).getProfile().getName()
							+ " joined the game!");
				}
			}
			...
			public void connected(ConnectedEvent event) {
				System.out.println(
						Hydrazine.infoPrefix + ((MinecraftProtocol) client.getPacketProtocol()).getProfile().getName()
		...
		if (Hydrazine.settings.hasSetting("socksproxy")) {
			...
			if (Hydrazine.settings.getSetting("socksproxy").contains(":")) {
				...
				if (socksFile.exists()) {
					...
					proxy = socksFactory.getProxies(Proxy.Type.SOCKS)[r
							.nextInt(socksFactory.getProxies(Proxy.Type.SOCKS).length)];
				} else {
					System.out.println(Hydrazine.errorPrefix + "Invalid value for switch -sp");
